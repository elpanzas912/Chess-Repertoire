-- Enable Drill tracking and persist per-opening Drill high scores.
...
ALTER TABLE public.training_activity_events
    ...
    CHECK (mode IN ('learn', 'practice', 'drill'));
...
BEGIN
    ...
    END IF;
    IF p_mode NOT IN ('learn', 'practice', 'drill') THEN
...
CREATE OR REPLACE FUNCTION public.save_drill_high_score(
    p_opening_slug TEXT,
    p_score INTEGER
)
RETURNS JSONB
...
BEGIN
    ...
    IF p_score < 0 OR p_score > 1000000 THEN
        RAISE EXCEPTION 'Invalid Drill score';
    END IF;

    INSERT INTO public.opening_progress (user_id, opening_slug, drill_high_score, updated_at)
    VALUES (user_uuid, p_opening_slug, p_score, NOW())
    ON CONFLICT (user_id, opening_slug) DO UPDATE SET
        drill_high_score = GREATEST(public.opening_progress.drill_high_score, EXCLUDED.drill_high_score),
        updated_at = NOW();
...
REVOKE ALL ON FUNCTION public.save_drill_high_score(TEXT, INTEGER) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.save_drill_high_score(TEXT, INTEGER) TO authenticated;
